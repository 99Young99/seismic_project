...
import  numpy
from Algorithms.varSem import varSem
...
def semanticGP(population, toolbox, ngen, tarSem, library, output_file=None, stats=None,
             halloffame=None, verbose=__debug__):
    ...
    fit_sv = toolbox.map(toolbox.evaluate, invalid_ind) # 通过定义的 evalSymbReg 函数， 计算每个个体的 fitness


    for ind, f_s in zip(invalid_ind, fit_sv):  # fitnesses, seman_vecs , ind: 个体表达式
        # print('ind: ', ind, ', f_s: ', f_s)
        ind.fitness.values=f_s[0] # (fitnesses, seman_vecs)
        ind.sem_vec = f_s[0][1] # seman_vecs
        # print(f_s[0][0]) # fitnesses

        evas = evas + len(ind)

    # 存储迭代的个体到 halloffame 中
    if halloffame is not None:
        halloffame.update(population)
    ...
    for gen in range(1, ngen + 1):
        ...
        # Select the next generation individuals, offspring
        offspring = toolbox.select(population, len(population))

        offspring = varSem(offspring, toolbox, tarSem, library)
        ...
        for ind, f_s in zip(invalid_ind, fit_sv):
            ...
            ind.sem_vec = numpy.array(list(f_s[0][1])) # 语义向量
            ind.sem_vec = np.nan_to_num(ind.sem_vec, nan=0.0) # 将其中的空值替换为 0
            evas = evas + len(ind)
            # print(np.shape(ind.sem_vec))
        ...
        # library.lib_maintain(population) # 从新维护 library 库
        ...
        if halloffame.items[0].fitness.values[0] < 0.01 or evas > 1e7:
            break

    return population, None, evas